...
export default function ComposeForm() {
  ...
  const instrumentOptions = useMemo(() => {
    ...
    for (const inst of [...DEFAULT_INSTRUMENTS, ...customInstruments])
    ...
    setSections((prev) =>
      prev.map((s) =>
        s.mode === "manual"
          ? {
              ...s,
              instruments: s.instruments.filter((id) => ids.includes(id)),
            }
          : s,
      ),
    );
  ...
  async function onCompose() {
    ...
    try {
      ...
      const composeRes = await fetch(`/api/jobs/${newJobId}/compose`, {
        method: "POST",
      });
      if (!composeRes.ok) {
      ...
      const { downloadUrl } = (await composeRes.json()) as {
        downloadUrl: string;
      };
    ...
    <div>
      ...
      <div className="row">
        <div>
          ...
          <div className="small">
            Each selected instrument will be composed as its own track.
          </div>
        </div>
        <div>
          ...
          <div className="kv">
            ...
            >
              <button
                ...
                disabled={!customInstrumentLabel.trim()}
              >
                Add
              </button>
            </div>
      ...
      <div className="row">
        <div>
          ...
          <div className="small">
            Define timestamps and choose which instrument tracks should be
            composed per section.
          </div>
        </div>
        <div className="sectionList">
          {sections.map((s) => (
            <div key={s.id} className="sectionCard">
              ...
              <div className="kv">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      updateSection(s.id, {
                        name: e.target.value as SectionName,
                      });
                    }}
                  >
                    {(
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateSection(s.id, { customName: e.target.value })
                    }
                  />
                </div>
              </div>
              ...
              <div className="kv">
                <div>
                  ...
                  <input
                    ...
                    value={s.startTime || ""}
                    onChange={(e) =>
                      updateSection(s.id, { startTime: e.target.value })
                    }
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    value={s.endTime || ""}
                    onChange={(e) =>
                      updateSection(s.id, { endTime: e.target.value })
                    }
                  />
                </div>
              </div>
              ...
              <div style={{ marginTop: 10 }}>
                <div className="small">
                  Which instrument tracks should be composed in this section?
                </div>
                <div className="inline" style={{ marginTop: 8 }}>
                  {(["all", "none", "manual"] as InstrumentSelectionMode[]).map(
                    (mode) => (
                ...
                {s.mode === "manual" ? (
                  <div className="inline" style={{ marginTop: 10 }}>
                    {selectedInstrumentIds.length === 0 ? (
                      <div className="small">
                        Select instruments above to enable manual section
                        selection.
                      </div>
                    ) : (
                      selectedInstrumentIds.map((id) => {
                        const label =
                          instrumentOptions.find((x) => x.id === id)?.label ||
                          id;
          ...
          <div>
            ...
            <div className="small">
              If you don&apos;t care about sections, set all section modes to{" "}
              <b>all</b>.
            </div>
          </div>
        </div>
